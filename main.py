...
settings = Settings()
WIN = pygame.display.set_mode((settings.width, settings.width))
...
class Spot:
	...
	def is_end(self):
		return self.color == settings.turq
	def reset(self):
		self.color = settings.white
...
def recostruct_path(came_from, current, draw):
...
def algorithm(draw,grid,start,end):
	...
	while not open_set.empty():
		...
		if current == end:
			recostruct_path(came_from, end, draw)
			end.make_end()
			return True
...
def draw_grid(win,rows,width):
	gap = width // rows
	for i in range(rows):
		pygame.draw.line(win, settings.grey,(0,i * gap),(width, i * gap))
		for j in range(rows):
			pygame.draw.line(win, settings.grey,(j * gap, 0),(j * gap, width))
